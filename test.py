...
# Initialize models using secrets
Settings.llm = Groq(model="llama3-70b-8192", api_key=st.secrets["llama"]["api_key"])
Settings.embed_model = FastEmbedEmbedding(model_name=st.secrets["embed_model"]["model_name"])

class ChatbotInterface:
    def __init__(self):
        self.collection_name = st.secrets["qdrant"]["collection_name"]
        self.client = QdrantClient(url=st.secrets["qdrant"]["url"])
...
def main():
    ...
    with col2:
        ...
        st.image(logo)
    st.title("Advanced Document Processing with LlamaIndex 📚")
